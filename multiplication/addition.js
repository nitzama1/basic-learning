// Addition Game with Progressive Levels
class AdditionGame {
    constructor() {
        this.level = 1;
        this.score = 0;
        ...
        this.correctInLevel = 0; // Count correct answers in current level
    ...
    generateAnswerOptions(correctAnswer) {
        ...
        while (options.length < 4) {
            ...
            if (rand < 0.5) {
                ...
                wrongAnswer = Math.floor(Math.random() * (levelMax * 2)) + 1;
            }
    ...
    processAnswer(isCorrect) {
        ...
        if (isCorrect) {
            ...
            this.streak = 0;
            this.streakEl.textContent = this.streak;
            this.correctInLevel = 0; // Reset level progress on wrong answer
        ...
        this.verticalSolution.innerHTML = `
            <div class="vertical-calc">
                ...
                <div class="calc-explanation">
                    ...
                    <p class="tip">💡 טיפ: מתחילים לחבר מהספרה הימנית ביותר</p>
                </div>
            </div>
        `;
    }
    ...
    resetProgress() {
        if (confirm('האם אתה בטוח שברצונך לאפס את כל ההתקדמות?')) {
            ...
            localStorage.removeItem('additionProgress');
    ...
    saveProgress() {
        try {
            ...
            localStorage.setItem('additionProgress', JSON.stringify(progress));
        } catch (e) {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('additionProgress');
            if (saved) {

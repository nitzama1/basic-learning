...
function generateQuestion() {
    ...
    // Auto-play the letter after a short delay
    setTimeout(() => {
        speakLetterName(currentLetter.letter);
    }, 500);
}
...
function handleAnswer(cardElement, selectedOption) {
    ...
    if (selectedOption.letter === currentLetter.letter) {
        ...
        // Play letter again to help them learn
        setTimeout(() => {
            speakLetterName(currentLetter.letter);
...
function speakLetterName(letterName) {
    ...
    const audio = new Audio(`/speak/${encodeURIComponent(letterName)}/he`);

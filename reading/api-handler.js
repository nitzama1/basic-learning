...
class APIHandler {
  ...
  buildPrompt(topic) {
    ...
    return `אתה מורה לעברית לכיתה ג'. צור קטע קריאה מעניין ושאלות הבנה.

דרישות לקטע הקריאה:
1. אורך: 120-150 מילים
2. רמת קושי: מתאים לילדים בכיתה ג' (גילאי 8-9)
3. נושא: ${selectedTopic}
4. משפטים קצרים וברורים (8-12 מילים למשפט)
5. אוצר מילים מתאים לגיל - מילים פשוטות ומוכרות
6. תוכן מעניין וחינוכי
7. ללא נקודות (כתיב חסר) - כמו בספרי לימוד לכיתה ג'
8. הטקסט צריך להיות קוהרנטי ועם עלילה או מסר ברור

דרישות לשאלות:
1. בדיוק 5 שאלות הבנה
...
4. שאלות שבודקות:
   ...
   - זכירת פרטים (2 שאלות)
   - הסקת מסקנות (1 שאלה)
5. אפשרויות התשובה צריכות להיות הגיוניות ולא מובנות מאליהן
6. כל האפשרויות צריכות להיות באורך דומה
